...
- **A10: shell/detail split on chat-subscribe + wireVersion 21.** Splits the chat-subscribe WS stream into a thin `ChatShellEvent` (header — session id, sequence number, kind, emittedAt, optional token counts, turn state — ~200 bytes JSON typical) followed by a heavy `ChatDetailEvent`. v21+ clients receive both frames per 100ms coalesced commit; the activity strip / sidebar renders off the shell immediately while the detail fills in the full body. Wire-version 20 and earlier clients keep receiving the legacy `WireChatSnapshot` JSON frame unchanged. On the synthesized 100-message burst fixture, the shell payload is **99% smaller** than the legacy snapshot (206B vs 102,845B). `iOSChatStore` adds `@Published shellSummary: ChatShellEvent?`. New `WireV21ShellDetailTests` covers round-trip, capability gate, back-compat, and the streaming-burst payload-size gate. iOS / Mac / Watch builds green.

- **E4: iOS daemon outbound relay client with background-lifecycle handling.** New `IOSRelayClient` opens an outbound WSS to the E2 relay Worker using the `iosTok` bearer + sid from the persisted E7 pairing record, exchanges X25519 handshakes, and seals/opens chat-class frames with XChaCha20-Poly1305 IETF. Background lifecycle handled per design doc §11: suspend cancels WS cleanly, foreground reopens with same `sid + iosTok`. Replay protection per §4.3, TLS 1.3 minimum, constant-time peer-pubkey compare. **Shared `RelayFrameCodec` + `HChaCha20.swift` land in `ClawdmeterShared/Relay/` — no libsodium-swift dependency needed; pure-Swift HChaCha20 (~90 LOC) verified against RFC §2.2.1 + libsodium vectors.** Cross-verifies byte-exact against the 7 test vectors in `infra/relay/test-vectors/`. 17 lifecycle + handshake tests; 14 cross-impl crypto tests in `RelayCodecCryptoTests`. APNS-wake entry point (`connectForAPNSWake()`) placeholder until E6 lands.
