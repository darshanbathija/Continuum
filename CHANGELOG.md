...
- **Security + privacy + known-limitations docs (E8, Gate 0).** Three new reference docs under `docs/` describe the secure-cloud relay + APNS gateway story for the end user. `docs/security.md` covers the trust model (local daemon → paired peer → untrusted Worker → sandboxed provider CLIs), the cryptographic primitives (X25519 ECDH + HKDF-SHA256 + XChaCha20-Poly1305), the key lifecycle (ephemeral keypairs per pairing; no long-lived identity keys in v1), the D22 per-peer bearer auth, the D21 mitigation suite on the APNS gateway (per-device rate limit, audit log, kill-switch), the Codex #5 device-token egress controls (SHA-256 hash before persistence, tenant binding, 410 cleanup, opt-out endpoint), the F3 HOME isolation shape, the audit-log redaction rules, and a pointer to the 14-scenario threat model in the design doc. `docs/privacy.md` enumerates every byte that leaves the user's Mac — relay envelopes (opaque to the operator), APNS gateway pushes (hashed tokens + sealed bodies), pricing snapshot fetch (read-only HTTPS), provider CLI telemetry (each owns its own egress), GitHub update check — plus what stays local (chat transcripts, code diffs, repo paths, JSONL ingest state, Keychain entries), the operator's visibility surface for both Workers, the backup-exclusion posture on the F2 SQLite event store, and the GDPR / CCPA deletion story that the PR #146 review-fix made real (the `wal_checkpoint(TRUNCATE)` after `deleteSession` is what makes the purge non-recoverable). `docs/known-limitations.md` enumerates what is NOT yet in main: the Mac/iOS clients for the relay + APNS gateway (E3 / E4 / E6), the Swift CryptoKit XChaCha20 nonce-size gap caught by the /verify pass on PR #151, the F3 daemon wire-up that enforces HOME isolation runtime invariants, the C2 `@Observable` migration that the post-C2 perf claims assume, the watchOS Tahoe-debt, the iOS launch-surface Tahoe-debt (62/100 critique), and the Mac Code-tab density follow-ups deferred from PR #149. All three are linked from the README under a new "Security and privacy" section between Analytics and Building Apple targets.
