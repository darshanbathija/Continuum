...
- **A10: shell/detail split on chat-subscribe + wireVersion 21.** Splits the chat-subscribe WS stream into a thin `ChatShellEvent` (header — session id, sequence number, kind, emittedAt, optional token counts, turn state — ~200 bytes JSON typical) followed by a heavy `ChatDetailEvent` (items + plan steps + sources + artifacts + codex todos + token totals). v21+ clients receive both frames per 100ms coalesced commit; the activity strip / sidebar renders off the shell immediately while the detail fills in the full body. Wire-version 20 and earlier clients keep receiving the legacy raw `WireChatSnapshot` JSON frame unchanged (back-compat). The dispatch branch is chosen ONCE per connection from the client's `wireVersion` field on the subscribe envelope. New `ChatStreamFrame` envelope discriminates `shell` / `detail` / `snapshot` cases in the v21+ wire; legacy v20 keeps the bare snapshot shape. `iOSChatStore` adds `@Published shellSummary: ChatShellEvent?` so the activity strip can paint without waiting for the detail. On the synthesized 100-message burst fixture, the shell payload is **99% smaller** than the legacy snapshot (206B vs 102 845B) — well above the ≥80% acceptance bar. New `WireV21ShellDetailTests` covers round-trip, capability gate, back-compat (v20 client rejects v21 envelope; v21 client falls back to bare snapshot decode), and the streaming-burst payload-size gate. iOS / Mac / Watch builds verified green.
