...
- **Archiving a worktree session now reclaims its disk.** Each worktree-mode session is a full ~54 MB checkout under `~/Clawdmeter/workspaces/`, and Archive previously only flipped an `archivedAt` flag — so checkouts piled up (measured 1.3 GB across ~25 stale worktrees on one machine). Archive now stops the session's runtime and moves its worktree folder to the **macOS Trash** (recoverable), then prunes git's worktree entry. The branch is kept, so committed work is safe; **Unarchive re-creates the checkout** from the branch. New `WorktreeManager.trashWorktree` / `reprovision` + `AgentControlServer.teardownRuntimeForReclaim`; wired through `AgentSessionRegistry.archive`/`unarchive` so every archive entry point (sidebar, context menu, daemon) reclaims uniformly.
- **Code sidebar: one clean hover-revealed archive icon.** Session rows had two stacked archive affordances on hover (a chipped one plus a bare one added later), which cluttered the row and could clip on the right. Consolidated to a single bare `archivebox` glyph (Conductor-style) revealed on row hover, with a subtle highlight when the cursor is on the glyph.
- **New harness chats took multiple seconds in a "Starting…" state before appearing.** Creating a Cursor/Grok/Gemini/Codex chat blocked the HTTP create response on the full ACP/headless child spawn + handshake, so the session (and its sidebar row + thread) only appeared once the subprocess was live. The Solo Chat create path now returns the session the instant the record + sandbox cwd + pinned store exist (sub-second) and spawns the bridge in the background (`deferBridgeStart`); the first `/send` echoes the user turn immediately, then awaits the in-flight spawn before delivering — so the chat is on the left and your message is visible right away, and only the model's response waits. A spawn failure surfaces as an error bubble in the thread instead of a blocked create. Frontier (broadcast) per-child spawns keep the synchronous path so a broadcast only opens once every column is live.
- **Your own message didn't show until the harness replied (solo chats).** Sending in a solo Cursor/Grok chat showed only the "…" thinking dots — the "You" bubble appeared only once the assistant's answer streamed back. The solo `handleSendPrompt` harness branch drove `bridge.prompt()` without first echoing the user turn into the store (ACP bridges don't record it themselves), unlike the Frontier child-send path which already echoed. It now appends the "You" bubble to the session store IMMEDIATELY on send, so it renders the instant you hit send. (The echo is also captured by the durable mirror below.)
- **Harness chat history was collapsing to a single message (durability bug).** sdkOnly chat stores (Cursor, Grok, Gemini, OpenCode, Codex-SDK, Antigravity) mirror their transcript to `~/Library/Application Support/Clawdmeter/sdk-chat-transcripts/<id>.jsonl` so it survives the daemon's idle-evict and app restarts. But the mirror used an incremental append-mode `FileHandle` write that silently failed after the first line — every one of the 262 on-disk mirrors had collapsed to a single message, so reopening a harness chat showed only one turn. The mirror now keeps the COMPLETE, id-deduped message set in memory (seeded from disk on store creation) and atomically rewrites the whole file on every change (`Data.write(.atomic)`), which can't truncate or drop turns. Harness transcripts are now fully durable across evict / restart / app-update / reinstall (the path is outside the app sandbox, which is disabled, so it's never wiped by an update). Claude & Codex-CLI chats were already durable via their provider-owned JSONLs; the session list (`sessions.json`) is atomic-written with an event-log replay backstop.
- **Chat header showed the wrong model for an opened session.** Opening an existing solo chat (e.g. Cursor / `cursor-auto`) rendered the header from the global composer's last-picked vendor+model (e.g. ChatGPT / `gpt-5.5`) instead of the session's own. `MacChatV2View.Header` now resolves the opened session and shows ITS provider+model; the composer pick is used only for a brand-new draft.
- **The first message of every new Claude chat was silently dropped** (second message onward worked). Two compounding cold-spawn causes, both found by driving a live `claude` PTY and inspecting the rollout JSONL:
  - **Startup modal eats the first prompt (the real root cause).** A fresh per-chat cwd makes Claude block on a confirmation modal that BOTH swallows the first `submitPrompt()` AND stalls the Remote-Control handshake until answered — so the "input live" marker never arrives and the daemon submits straight into the modal. The dominant variant is **"New MCP server found in this project: MCP_DOCKER"**: Claude walks up from the nested chat-cwd to the user's `~/.mcp.json` and prompts for approval. This fires in interactive mode **even under `--strict-mcp-config`** (the v0.31.10 flag only gates which servers load, not the discovery prompt) and is not suppressible via the per-project enabled/disabled keys. `ClaudePtyHost.waitUntilReady()` now detects the modal (MCP-approval or folder-trust) and dismisses it with **Esc** — which cancels either prompt, **keeps plan mode**, and lets the handshake complete — before waiting for input-ready. Verified end-to-end: the opening prompt now lands in the rollout and gets a reply.
  - **Readiness fired too early.** Even with no modal, the cold-spawn wait keyed on output going "stable" (~1.1s, when the Ink TUI finishes its first render) rather than raw-mode entry (~2.5s), so the first submit could land ~1.4s early. `waitUntilReady()` now gates on the Remote-Control "active" marker plus a string-independent time floor, and never returns while a modal is still on screen.
- **One session rendered as two workspace tabs.** A persistence race had written a duplicate session-id into `sessions.json`. `AgentSessionRegistry.load()` now collapses duplicate ids on load (keep-first), and `WorkspaceTabStrip` defensively de-dupes the render list so a stray duplicate can never split into two tabs.
- Bumps `VERSION` / `MARKETING_VERSION` 0.31.10 → 0.31.11, and `CURRENT_PROJECT_VERSION` 210 → 218.
