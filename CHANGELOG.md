...
## [0.26.2 build 131] - 2026-05-24 — Code V2 follow-ups: cross-platform Watch build, app-scoped outbox, composer routing, test isolation (`darshanbathija/code-v3`)

Five tidy-ups against the v0.26.0 Code V2 ship that surfaced in post-merge review. None is user-visible on its own, but together they close the three "next session would hit this" risks: the Watch target was failing to build, the iOS outbox was created per-session-detail (so iPad multi-window or rapid navigation could race the persisted `outbox.json`), and the most common iOS write paths (composer send, refine, approve-plan) were going straight to `AgentControlClient` instead of through the outbox the user can see in `iOSOutboxPane`.
...
- **`TODOS.md` Code V2 follow-up brief moved to "Completed v0.26.x"**: the four items I shipped (workspace store, outbox, MagicDNS, workbench tabs) plus the five follow-ups in this build. Remaining open items (APNS-driven outbox flush, server-side TLS termination, workspace archive UI) stay in the "v0.6 / v1.0 deferrals" section.
