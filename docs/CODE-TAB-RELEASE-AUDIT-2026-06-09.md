...
## 2026-06-10 Addendum — Post-Merge Verification Pass (main `d215a7c5`)

PR #286 merged the audited work onto `main`. This pass re-baselined clean `main`, ran the first user-authorized live-spend provider smokes, ran the first full-class UI pass (all 40 `CodeTabHoverShortcutUITests`), and fixed everything those first-evers exposed.

| Element | Intended behavior | Prior behavior | Fix applied | Current status |
| --- | --- | --- | --- | --- |
| OpenCode send → reply (live) | A prompt sent through the Code tab to `opencode serve` streams the reply into the transcript and terminates the turn. | Dead end-to-end on opencode ≥1.16 (and the SSE consume loop had never delivered a live event on any version): reply never rendered, Stop state never cleared. Mock tests were green because they bypass transport/parse. | Four fixes in `OpencodeSSEAdapter`: 1.16 event projection (`message.updated`/`message.part.*`/`session.idle`, role-gated, completion-time tokens/model), per-directory `/event` streams (LRU cap 32 + envelope-id dedupe), `Accept-Encoding: identity`, dispatch-on-complete-JSON instead of blank-line terminators. | `testOpenCodeLiveDrive` PASSES live (5.96s, reply token verified). 18/18 ChatSendPerProviderTests, 30/30 OpencodeSSEAdapterTests, 11/11 OpencodeSendTests. |
| Codex / Antigravity / Grok (live) | Real prompt → streamed reply → turn completion. | Never executed live (gated, no spend authorization). | None needed. | All three PASS live (20.7s / 17.6s / 23.7s). |
| Cursor (live) | Auth/quota errors surface at spawn time. | Never executed live. | None needed — behavior matched intent. | SKIP with surfaced account quota state ("Upgrade your plan to continue"); rerun after plan upgrade. |
| Center header (density, More-actions, checkpoint flows) | Header controls expose stable AX identifiers (`code.header.density`, `code.header.more-actions`, …). | A container-level `code.center.header` identifier (added 2026-06-09 ~22:00 for the header-switch test) stomped every child identifier — 7 UI tests red, header controls unaddressable for AX. | `.accessibilityElement(children: .contain)` on the header container (`CenterThread.swift`). | All 7 affected tests pass; the header-switch test still passes via the `code.center.header.state` marker. |
| Repo settings → "New session here" | Opens the Code launcher (instant quick-spawn belongs to the adjacent `+`). | Silently became an instant quick-spawn: the launcher behavior had only ever been an accidental fallback (unresolvable workspace repo keys), and the key-canonicalization fix removed the accident. | Menu row wired to `prepareNewSession(in:)` (`SidebarPane.swift`). | Test passes; launcher opens with Start action. |
| Expanded composer editor → Save Prompt | Saves title+body to the presentation store. | Test red from cross-run contamination: composer drafts persist in `UserDefaults.standard` keyed by the fixture's fixed session UUID, accumulating every prior run's typed text. | Test-side NSArgumentDomain overrides for the seeded draft keys; permanent failure-diagnostics attachment (persisted JSON + rendered editor values) added to the test. | Test passes. Product draft persistence unchanged (it is a feature). |
| Plan pane → Approve (rendered click) | Right-pane Approve creates a checkpoint and approves the plan (pending → approved visual transition). | Compiled but execution-pending (desktop was locked on 2026-06-09). | None — executed as written. | PASSES (31.9s). |
| Test orchestration | UI runs own the machine. | App-hosted unit/live xctest bundles boot a second Continuum whose daemon binds fixed ports 21731/21732, corrupting concurrent XCUITest runs. | Process rule documented in `docs/CODE_TAB_AUDIT.md`. | Exclusive reruns deterministic. |
| Permission-mode ⌘⇧1-4 shortcuts | App-wide shortcuts flip the composer permission mode. | Failing in every run after 03:16 — Shottr (screenshot utility launched at 03:16:42) registers global ⌘⇧1/⌘⇧2 hotkeys that swallow the keystrokes system-wide. | None (external interference, product behavior correct); Shottr added to the documented UI-run interference list. | Test passes in 12.2s with Shottr quit; Shottr relaunched after. |
| Daemon route layer (live) | Real spawns/sends through `/sessions` and `/chat-sessions` for every provider. | Never executed live. | None needed (one env-gate doc fix: daemon-side spend gate needs `CLAWDMETER_LIVE_PROVIDER_TESTS=1`). | All four gated route smokes PASS: 5-provider worktree spawn smoke (incl. live Claude), Cursor route, OpenRouter live send, Frontier gemini broadcast via agy. |
| Full Code-tab UI class (40 tests) | Whole class green in one pass. | Never run as one class; first run exposed 10 reds. | The three fixes above + environmental root-causes for the rest. | 38/40 in-class; the 2 in-class failures reproduce green focused (picker flake; Shottr hotkey theft). Every test green on current code. |
