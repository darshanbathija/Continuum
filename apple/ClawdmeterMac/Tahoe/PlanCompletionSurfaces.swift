...
struct RepoFilePickerView: View {
    ...
    private var filteredFiles: [RepoFileMatch] {
        RepoFileSearch.matches(
            query: query,
            files: files,
            recents: presentationStore.snapshot.recentPathActions
        )
    }
    ...
    private func loadFiles() async {
        ...
        let loaded = await Task.detached(priority: .utility) {
            Self.gitFiles(repoRoot: repoRoot)
        }.value
        files = loaded.files
        error = loaded.error
        isLoading = false
    ...
    nonisolated private static func gitFiles(repoRoot: String) -> (files: [String], error: String?) {
        let process = Process()
...
private struct RepoFileMatch: Identifiable, Hashable {
    var path: String
    var line: Int?
    var score: Int
    var isRecent: Bool
...
enum RepoFileSearch {
    fileprivate static func matches(query: String, files: [String], recents: [String], limit: Int = 160) -> [RepoFileMatch] {
