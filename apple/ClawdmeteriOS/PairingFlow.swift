...
// Extracted from the now-retired iOSSessionsView.swift (v0.11). Kept as a
// standalone file because PairingCTAButtons.swift still presents this sheet.
// The Tahoe Code surface (IOSCodeView) doesn't host pairing inline.
...
struct PairingFlow: View {
    ...
    private func applyChallenge(_ challenge: PairingChallenge) {
        ...
        // v0.14.0 (plan v2.1): persist Design routing values too when the
        // pairing payload carries them. Older Mac builds omit these fields
        // and iOS shows the Design empty-state until re-pair.
