...
public final class Pricing: @unchecked Sendable {
    ...
    /// Lock-guarded `var` (not `let`) so `PricingUpdater` can hot-swap the table
    /// from a daily background refresh without an app restart, while readers
    /// (cost/isPriced) run concurrently on other threads.
    private let lock = NSLock()
    private var models: [String: ModelRates]
    ...
    public init(bundle: Bundle) {
        // Prefer the daily-refreshed runtime cache (see PricingUpdater) over the
        // embedded snapshot, falling back to the bundled pricing.json when the
        // cache is absent (offline / before the first refresh).
        if let cacheURL = Self.cacheURL,
           let data = try? Data(contentsOf: cacheURL),
           let table = Self.decodeTable(from: data) {
            let merged = Self.applyingBundledOverrides(to: table, bundle: bundle)
            self.models = merged.models
            self.modelKeys = merged.keys
            self.logger.info("Pricing loaded \(merged.models.count, privacy: .public) models from runtime cache")
            return
        }
        guard let url = bundle.url(forResource: "pricing", withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let table = Self.decodeTable(from: data) else {
            self.logger.error("Pricing: could not load pricing.json (cache or bundle)")
            self.models = [:]
            self.modelKeys = []
            return
        }
        ...
        self.models = merged.models
        self.modelKeys = merged.keys
        self.logger.info("Pricing loaded \(merged.models.count, privacy: .public) models from bundle")
    }

    /// Runtime cache location written by `PricingUpdater` (daily refresh).
    public static var cacheURL: URL? {
        FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            .appendingPathComponent("Clawdmeter/pricing-cache.json")
    }

    /// Decode a `{ _meta, models }` snapshot into the lookup table.
    private static func decodeTable(from data: Data) -> (models: [String: ModelRates], keys: [String])? {
        guard let payload = try? JSONDecoder().decode(SnapshotPayload.self, from: data) else { return nil }
        var byKey: [String: ModelRates] = [:]
        for (key, raw) in payload.models { byKey[key] = Self.toModelRates(raw) }
        return (byKey, Array(byKey.keys).sorted(by: { $0.count > $1.count }))
    }

    private static func applyingBundledOverrides(
        to table: (models: [String: ModelRates], keys: [String]),
        bundle: Bundle
    ) -> (models: [String: ModelRates], keys: [String]) {
        guard let overrides = Self.decodeBundledOverrides(bundle: bundle), !overrides.isEmpty else {
            return table
        }
        var merged = table.models
        for (key, raw) in overrides {
            merged[key] = Self.toModelRates(raw)
        }
        return (merged, Array(merged.keys).sorted(by: { $0.count > $1.count }))
    }

    private static func decodeBundledOverrides(bundle: Bundle) -> [String: RawModel]? {
        guard let url = bundle.url(forResource: "pricing-overrides", withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let payload = try? JSONDecoder().decode(OverridesPayload.self, from: data) else {
            return nil
        }
        return payload.overrides
    }

    /// Hot-swap the pricing table from freshly-fetched snapshot data. Called by
    /// `PricingUpdater` after a successful daily refresh.
    @discardableResult
    public func reload(from data: Data) -> Bool {
        guard let table = Self.decodeTable(from: data) else { return false }
        lock.lock()
        self.models = table.models
        self.modelKeys = table.keys
        lock.unlock()
        // Invalidate the B3 resolution LRU so post-reload lookups don't keep
        // serving pre-reload rates for already-seen model names.
        cacheLock.lock()
        resolutionCache.removeAll()
        resolutionInsertionOrder.removeAll()
        cacheLock.unlock()
        self.logger.info("Pricing hot-reloaded \(table.models.count, privacy: .public) models")
        return true
    }
    ...
    private func rates(for model: String) -> ModelRates? {
        ...
        // Cold path: resolve against a lock-snapshot of the table so a
        // concurrent PricingUpdater hot-reload can't tear the dictionary
        // mid-lookup (Dictionary is COW, so the snapshot is cheap).
        lock.lock()
        let models = self.models
        let modelKeys = self.modelKeys
        lock.unlock()
        let resolved: ModelRates? = {
            for alias in Self.aliases(for: model) {
                if let exact = models[alias] { return exact }
                let strippedAlias = Self.stripDateSuffix(alias)
                if strippedAlias != alias, let m = models[strippedAlias] { return m }
                for candidate in modelKeys where alias.hasPrefix(candidate) {
            ...
            // Provider-prefix fallback: bare opencode names like `grok-4.3`
            // resolve to LiteLLM's `xai/grok-4.3` key instead of pricing at $0.
            if !model.contains("/") {
                for prefix in ["xai/"] {
    ...
    /// the model catalog reloads (e.g. PricingUpdater's daily hot-reload,
    /// which calls this so the LRU doesn't keep serving pre-reload rates).
    ...
    private static func aliases(for model: String) -> [String] {
        ...
        let bareCursorModel: String
        if trimmed.lowercased().hasPrefix("cursor/") {
            bareCursorModel = String(trimmed.dropFirst("cursor/".count))
        } else {
            bareCursorModel = trimmed
        }

        switch bareCursorModel.lowercased() {
        case "composer-2.5", "composer-2.5-fast":
            // Cursor hook logs emit Composer ids, while the public price source
            // exposes the underlying model through OpenRouter's Kimi K2.5 row.
            // Keep this as an alias instead of renaming analytics lanes so the
    ...
    private struct OverridesPayload: Decodable {
        let overrides: [String: RawModel]
    }
