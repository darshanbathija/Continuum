...
public final class Pricing: @unchecked Sendable {
    ...
    private func rates(for model: String) -> ModelRates? {
        // Snapshot the table under the lock so a concurrent hot-reload can't
        // tear the dictionary mid-lookup. (Dictionary is COW; this copies only
        // the struct header until the next reload replaces it.)
        lock.lock()
