...
/// cheap to deserve a toast (effort bumps, mode flips). When `trigger` changes,
/// a success-tinted ring fades in over the content and fades out ~0.6s later —
/// enough to confirm "that landed" without stealing focus or spamming toasts.
///
/// Single-shot by construction (no `repeatForever`), safe under Reduce Motion;
/// in that mode the scale is dropped and only opacity changes.
