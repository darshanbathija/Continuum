...
public struct ModelCatalog: Codable, Sendable {
    ...
    ///
    /// Fable 5 is intentionally NOT offered as a selectable Claude model: it is
    /// subject to US export restrictions, so it is excluded from the Code and
    /// Chat model selectors. Its pricing row stays in pricing.json so historical
    /// analytics for any past Fable usage still resolve a $/token cost.
    public static let bundled = ModelCatalog(
        claude: [
            ModelCatalogEntry(id: "claude-opus-4-8-1m",        provider: .claude, displayName: "Opus 4.8 (1M)",   cliAlias: nil,      supportsThinking: true,  supportsEffort: true,  contextWindow: 1_000_000, recommendedFor: "Hardest tasks",  badge: "1M"),
